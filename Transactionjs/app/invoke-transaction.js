 ...
 */
var invokeChaincode = function (targets, channelName, chaincodeName, fcn, args) {
	...
	return helper.getClient().then(_client => {
		...
		if(user !== null){
			logger.info('Current invoke user is: ' + user.getName());
		}
		...
		if (sendTransaction_results instanceof Error) {
			...
			return {status: "Invoke the chaincode Successfully", metadata: pass_results[0][0].response.payload.toString('utf8')}
		} else {
